...
### 2026-07-16 — Security Hardening + AI Improvements

**Security (ingestor/api-gateway):**
- Hardcoded HMAC fallback removed from OAuth state — app panics without `JWT_SECRET`
- `GIN_MODE` defaults to `release` (no stack trace leakage in HTTP error responses)
- 4 MB request body cap via `RequestBodyLimit` middleware
- PostgreSQL `sslmode` defaults to `require` (set `POSTGRES_SSLMODE=disable` for local dev)
- `ChangePassword` now enforces full strength rules (uppercase + digit + special char)
- `SetTrustedProxies` reads `TRUSTED_PROXIES` env var

**Security (ui/vercel):**
- `vercel.json` now delivers `Content-Security-Policy`, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy` headers on every response

**AI Core (ai-core):**
- CVE matching replaced with BM25+IDF multi-signal scoring engine — vendor alias expansion for 18 vendors, multi-field weighted scoring, CVSS boost; no more noisy random fallback

**UI:**
- Alert details Watson AI card: three states — `InlineLoading`+`SkeletonText` while re-analyzing, empty state with Watson icon when no analysis, full content when ready
- Design token unification: all page gaps use `$tok-page-content-gap`; stale `--page-*` CSS vars removed
- Input character limits with Carbon counters on ticket fields
